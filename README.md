...
#### UML 
**UML Процесс онлайн-записи на прием**  
<img width="887" height="691" alt="image" src="UML-001.png" />

```
@startuml
title [UML-001] -[Процесс онлайн-записи на прием]
actor Пациент
...
```
**UML Процесс онлайн-записи на прием c альтернативным сценарием, блокировкой слота и retry**
<img width="887" height="691" alt="image" src="UML-002.png" />

```
@startuml
title [UML-002] -[Процесс онлайн-записи на прием с alt]
...
AS->>DB: INSERT slot_lock (TTL = 30 сек) (временная блокировка слота)
...
alt Слот свободен
    ...
    DB-->>AS: Слот занят
    AS-->>API: Ошибка SLOT_NOT_AVAILABLE
    API-->>WA: Показать сообщение "Слот занят"
    WA-->>Пациент: Предложить выбрать другой слот
end
...
loop Retry отправки (до 3 попыток)
NS->>Пациент: SMS подтверждение записи
end
